...
# We'll call the Django API endpoint we already have:
...
def ratio_slider(id_, value=0.2):
    # UI takes 0–100, backend expects 0–1 (we'll divide by 100 in the callback)
    return dcc.Slider(
...
def on_predict(n_clicks, mavg, t1, t2, dow, cnt, adm, dis, trf, icu, emer, elderly, longstay, hosp, dept, shift):
    ...
    # Format if numeric; otherwise just show as-is
    try:
